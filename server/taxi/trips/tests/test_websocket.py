...
def create_trip(
    ...
    return Trip.objects.create(pick_up_address=pick_up_address,
...
class TestWebSocket:
    ...
    # async def test_and_can_receive_broadcast_messages(self,settings):
    #     settings.CHANNEL_LAYERS = TEST_CHANNEL_LAYERS
    #     communicator = WebsocketCommunicator(
    #         application = application,
    #         path = '/taxi/'
    ...
    async def test_join_driver_pool(self,settings):
        settings.CHANNEL_LAYERS = TEST_CHANNEL_LAYERS
        _ , access = await create_user('testuser@example.com','passWord','driver')
        communicator = WebsocketCommunicator(
            application = application,
            path = f'/taxi/?token={access}'
        )
    ...
    async def test_request_trip(self,settings):
        settings.CHANNEL_LAYERS = TEST_CHANNEL_LAYERS
        user , access = await create_user('testuser@example.com','passWord','rider')
        communicator = WebsocketCommunicator(
            application = application,
            path = f'/taxi/?token={access}'
        )
        ...
        await communicator.send_json_to({
            'type': 'create.trip',
            'data': {
                'pick_up_address': '123 Ikorodu',
                'drop_off_address': '321 Ojota',
                'rider': user.id,
            },
        })
        response = await communicator.receive_json_from()
        ...
        assert response_data['pick_up_address'] == '123 Ikorodu'
        assert response_data['drop_off_address'] == '321 Ojota'
